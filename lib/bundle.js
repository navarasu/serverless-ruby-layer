...
function bundleGems()  {
  if(fs.pathExistsSync(path.join(this.ruby_layer,'Gemfile'))) {
    let current_file = fs.readFileSync(path.join(this.servicePath,'Gemfile')).toString()
    let last_file = fs.readFileSync(path.join(this.ruby_layer,'Gemfile')).toString()
    this.changed = current_file != last_file
  }
  if (this.changed && fs.pathExistsSync(path.join(this.ruby_layer,'Gemfile.lock'))){
    current_file = fs.readFileSync(path.join(this.servicePath,'Gemfile.lock')).toString()
    last_file = fs.readFileSync(path.join(this.ruby_layer,'Gemfile.lock')).toString()
    this.changed = current_file != last_file
  }

  if (this.changed){
    this.cli.log("Gemfile changed, re-bundling")
