...
function bundleInstall(){
  ...
  if (this.options.use_docker) {
    ...
    if (this.options.additional_files) {
      this.cli.log("Packing the additional files from the specified path")
      let opt_path = path.join(this.build_path)
      this.options.additional_files.forEach(file_to_copy => {
        // if the last characte is a / then we assume that we want to copy the whole directory
        if(file_to_copy.slice(-1) === '/'){
          opt_path = this.build_path
        }
        ps=docker(['cp','-L', container_id+':'+file_to_copy, opt_path],options,this.cli)
      })
    }
...
function zipBundleFolder() {
  ...
  if (this.options.addtional_files) {
    zipping_message+=" and additional files"
  }
